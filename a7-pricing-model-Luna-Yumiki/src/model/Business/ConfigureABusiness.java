...
public class ConfigureABusiness {
  ...
  public static Business createABusinessAndLoadALotOfData(String name, int supplierCount, int productCount,
    ...
    System.out.println("loadSuppliers completed......");

    // Add Products +
    loadProducts(business, productCount);
    System.out.println("loadProducts completed......");

    // Add Customers
    loadCustomers(business, customerCount);
    System.out.println("loadCustomers completed......");

    // Add Order
    loadOrders(business, orderCount, itemCount);
    System.out.println("loadOrders completed......");
  ...
  static void loadProducts(Business b, int productCount) {
    ...
    // pickedSuppliers is to store the selected (5 out of 30) suppliers.
    ArrayList<Supplier> pickedSuppliers = new ArrayList<Supplier>();

    // radomSupplier is 1 randomly picked supplier, it checks whether it is already inside the pickedSuppliers. 
    // this check is to avoid repeat.
    while ((pickedSuppliers.size() < productSupplier)) {
      Supplier randomSupplier = supplierDirectory.pickRandomSupplier();
      if (pickedSuppliers.indexOf(randomSupplier) == (-1)) pickedSuppliers.add(randomSupplier);
    }

    // for (Supplier supplier : pickedSuppliers) {
    //   System.out.println(supplier);
    // }

    b.pickedSuppliers.setSuplierList(pickedSuppliers);

    // add 20 Products to each selected suppliers
    for (Supplier supplier : pickedSuppliers) {
  ...
  static void loadOrders(Business b, int orderCount, int itemCount) {
    ...
    SupplierDirectory sd = b.getPickedSupplierDirectory();

    for (CustomerProfile randomCustomer : cd.getCustomerList()) {
      ...
      // e. 5 to 15 orders per customer
      int randomOrderCount = getRandom(5, orderCount);
      for (int index = 0; index < randomOrderCount; index++) {
